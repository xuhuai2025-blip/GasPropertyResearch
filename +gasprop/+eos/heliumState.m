...
if any(~isfinite(T(:)) | T(:)<20 | T(:)>1500) || any(~isfinite(rho(:)) | rho(:)<=0 | rho(:)>500)
    error('gasprop:HeliumEOSRange','He EOS wrapper requires 20..1500 K and 0 < rho <= 500 kg/m3.');
end
...
if any(p<=0 | p>2e9 | cv<=0 | prho<=0 | c2<=0)
    error('gasprop:HeliumEOSState','Invalid or out-of-pressure-range normal-fluid helium state.');
end
